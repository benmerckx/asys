...
class Socket extends PlainSocket {
	override function createSocket() {
		socket = cast #if nodejs null #elseif hxssl new HxsslSocket() #elseif php new php.net.SslSocket() #elseif java new java.net
			.SslSocket() #elseif (haxe_ver > 3.210) #if (cpp || neko) new sys.ssl.Socket() #else null;
			throw 'Not supported on this platform' #end #else null;
			throw 'Not supported on this platform' #end;
	}
	...
	public static function upgrade(socket:PlainSocket):Socket {
		...
		socket.socket.sock = (untyped java.javax.net.ssl.SSLSocketFactory.getDefault()).createSocket(socket.socket.sock, socket.socket.sock
				.getInetAddress().getHostAddress(), socket.socket.sock.getPort(), true);
	...
	override public function connect(host:Host, port:Int):Promise<Noise> {
		...
		setStreams();
		socket.on('error',
		function(err:js.Error) trigger.trigger(Failure(Error.withData(err.message, err))));
		socket.on('connect', function() trigger.trigger(Success(Noise)));
